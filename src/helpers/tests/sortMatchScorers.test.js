import sortMatch, { basicScorer, objectScorer } from '../sortMatchScorers';
...
describe('Helper: sortMatchScorers', () => {
  ...
  describe('object scorer', () => {
    ...
    it('should return correct score if a single matching key returns a scoring value', () => {
      expect(objectScorer({
        item: { name: 'Janet Jones', age: 29 },
        objectPattern: { name: 'matches' },
        scorer: mockScorer
      })).toEqual(100);
      expect(mockScorer).toHaveBeenCalledTimes(1);
    });

    it('should accumulate score when multiple matching keys have scoring values', () => {
      expect(objectScorer({
        item: { name: 'Janet Jones', age: 29, address: '123 Fake St' },
        objectPattern: { name: 'matches', age: 29, whatever: 'nope' },
        scorer: mockScorer
      })).toEqual(200);
    ...
    it(`should return 0 if no matching keys have scoring values`, () => {
      ...
      expect(objectScorer({
        item: { name: 'Janet Jones', age: 29, address: '123 Fake St' },
        objectPattern: { name: 'no match', age: 44, whatever: 'nope' },
        scorer: mockScorer
      })).toEqual(0);
      expect(mockScorer).toHaveBeenCalledTimes(2);
    });

    it(`should return correct score when some matching keys have scoring values`, () => {
      mockScorer = jest.fn(() => 0);
      mockScorer.mockReturnValueOnce(100);
      expect(objectScorer({
        item: { name: 'Janet Jones', age: 29, address: '123 Fake St' },
        objectPattern: { name: 'matches', age: 44, whatever: 'nope' },
        scorer: mockScorer
      })).toEqual(100);
      expect(mockScorer).toHaveBeenCalledTimes(2);
    });
